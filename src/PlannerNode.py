...
class PlannerNode(object):
  ...
  def __init__(self): 
    ...
    self.fat_map = fatten_map (self.map_msg, 30)    
    self.map_pub.publish (self.fat_map)
    
    print("Done beefening map")
    
    self.prm = PRM.PRM (self.fat_map, 150, 600)
    ...
    for edge in self.prm.graph.edges (data = "weight"):
      ...
      p1[1] = self.fat_map.info.height - p1[1]
      p2[1] = self.fat_map.info.height - p2[1]

      p1 = Utils.map_to_world (p1 + [0], self.fat_map.info)
      p2 = Utils.map_to_world (p2 + [0], self.fat_map.info)
  ...
  def plan_cb (self, req):
    ...
    source = np.array ([req.poses[0].position.x, req.poses[0].position.y]).reshape (2)
    ...
    if plan:
      ...
      for node in plan:
        ...
        node[1] = self.fat_map.info.height - node[1]
        node = Utils.map_to_world (node, self.fat_map.info)
