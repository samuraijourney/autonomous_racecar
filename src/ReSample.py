...
class ReSampler:
  ...
  def resample_low_variance(self):
    ...
    initval = np.random.uniform() * (1.0/self.particles.shape[0]) # Choose an initial value from [0, 1/M]
    vals    = initval + self.step_array # Get the bin partitions
    cumwt   = np.cumsum(self.weights) # Get the cumulative sum of the weights
    proposal_indices = np.searchsorted(cumwt, vals, side='left') # Get indices according to bin weights
